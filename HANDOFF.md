...
Depois da aplicação, os advisors apontaram duas FKs novas sem índice. Foi criado o teste de contrato e a migration `20260729095743_add_fluency_fk_indexes.sql`, aplicada também em produção, cobrindo `private.fluency_task_responses(issue_id)` e `public.fluency_task_submissions(attempt_id)`. O cache do PostgREST foi recarregado. Chamadas anônimas reais aos dois endpoints que falhavam agora retornam `401`, não `404/PGRST205`, como esperado porque o acesso é exclusivo de `authenticated`; grants, RLS, contagens e índices foram verificados diretamente no banco.

Próximo passo concreto: fazer `Ctrl+F5` no dashboard autenticado (ou fechar e reabrir a PWA) e confirmar que Início/Progresso carregam sem erro de console. Em seguida, executar o QA manual da tradução contextual na extensão com uma palavra ambígua. O único portão ainda não provado nesta sessão é a renderização autenticada no navegador do usuário; não alterar RLS ou conceder acesso a `anon` para contornar esse teste.

## Handoff Codex — tradução contextual ponta a ponta (2026-07-29)

Corrigido localmente o contrato de tradução para que o significado no contexto seja canônico em todos os consumidores. O popup agora aguarda a resolução contextual antes do primeiro teste; salvamento e enriquecimento tardio promovem a tradução contextual; Web Reader, Histórias e popup do Reader resolvem a palavra dentro da frase; e o Estudo repara cards antigos ao promover `word_pt` contextual para `words.translation`/card. Assim, em `this is gross`, o fluxo deve persistir e exibir “nojento/repugnante”, não o fallback isolado “bruto”. A regra vale genericamente para palavras polissêmicas, não contém exceção hardcoded para `gross`.
...
QA visual local não foi executado: o navegador integrado bloqueou `file://` por política e não há extensão Chrome carregada nessa superfície. Próximo passo concreto: recarregar a extensão local no Chrome e testar uma palavra ambígua em uma frase real, confirmando popup, primeiro teste, Cofre/dashboard e card de estudo. O código foi publicado na `main` pelo PR `#18`, squash `1fa8841`. Os checks obrigatórios passaram, incluindo replay integral em PostgreSQL efêmero e release smoke. As migrations de fluência foram aplicadas ao Supabase posteriormente, conforme o handoff acima; a tradução contextual ainda não deve ser tratada como QA visual concluído.

## Handoff Codex — fundação de fluência real (2026-07-28)

Implementado localmente o primeiro corte seguro do novo método: contrato A1–B2 em `docs/CONTRATO_FLUENCIA_A1_B2_2026-07-28.md`, motor puro em `dashboard/js/core/fluencyCheck.js`, rota acessível `fluency-check` dentro de Progresso e migration expand-only `20260728133452_learning_task_attempts.sql`. A evidência é deliberadamente separada de cards, reviews, FSRS, XP, ofensiva e liga. Escritas do cliente são não autoritativas, idempotentes, owner-only e aceitam somente metadados em allowlist; não armazenam áudio, transcrição ou resposta livre.

Testes `test:fluency`, `test:p0-a` e design-system estão verdes. O gate comportamental SQL foi escrito em `tests/db/learning-task-attempts.sql`, conectado ao replay e aprovado no PostgreSQL efêmero do GitHub Actions no PR `#18`. O seed do catálogo foi reconciliado com a tabela autoritativa e `test:release` agora inclui os contratos de seed, autoridade, Edge Function, cliente e UX de fluência. A verificação visual automatizada local foi bloqueada pela política do navegador e continua pendente. As migrations e a Edge Function foram aplicadas no Supabase remoto em 29/07, conforme o handoff de produção acima.

Próximo passo concreto: validar a tela autenticada após recarregar o dashboard e executar a calibração humana com respostas-âncora antes de habilitar qualquer atribuição de nível na UI. Não integrar o resultado a FSRS, XP ou liga.
