...
Próximo passo concreto: validar `supabase/migrations/20260728133452_learning_task_attempts.sql` em PostgreSQL 17 efêmero com testes de RLS, grants, replay idempotente e `idempotency_conflict`; depois pedir autorização para aplicar no projeto `qnutoswrufznztoznlql`. Só após esse portão, criar o catálogo versionado e a entrega server-side de estímulos inéditos. Não habilitar atribuição de nível na UI antes do avaliador autoritativo e da calibração humana.
