...
| 2026-09-24 | Listening distingue faixa de áudio selecionada (`audio_track`), confirmação do aluno (`user_confirmed`) e estimativa pela legenda automática original (`caption_asr`). Legenda manual/traduzida, idioma do título e idioma alvo não são evidência de áudio; faixa de áudio selecionada prevalece e múltiplas faixas ambíguas suspendem a estimativa. | O contador pode funcionar em vídeos de áudio único sem `getAudioTrack`, preservando a proveniência e evitando crédito automático para vídeos dublados com legenda de outro idioma. |
| 2026-09-21 | Jogos e mini-jogos foram retirados da superfície do produto; a prática principal permanece contextual, orientada por revisão e chunks. A rota antiga `game` apenas redireciona para Aprender, enquanto migrations e eventos históricos permanecem para compatibilidade de dados. | Evita competir por atenção com o ciclo de aprendizagem e não apaga histórico relacional nem exige migration destrutiva. |
| 2026-09-21 | Cards novos persistem no `ai_chunks` existente a ocorrência real como `is_context` e a unidade que deve ser aprendida como `is_learning_unit`; variações ficam subordinadas a esse contexto, sem criar uma tabela ou migration nova. | Mantém a compatibilidade com cards legados e impede que a tela trate exemplos genéricos como se fossem a frase capturada do vídeo. |
| 2026-09-21 | A interface de captura, Home e estudo segue uma linguagem editorial por função: uma ação dominante, conteúdo antes de decoração, superfícies e separadores antes de cards empilhados, e rótulos verbais antes de emoji/ícones ambíguos. | Reduz a aparência genérica de produto gerado e torna contexto, estado e próximo passo legíveis sem depender de familiaridade visual. |
| 2026-09-22 | As referências visuais atualizadas do produto passam a ser o norte da Home e da Sessão de estudo: tema escuro azul-marinho, navegação superior enxuta, ação do dia dominante, métricas em linha e card de estudo com frase, áudio, tradução, sentido contextual e quatro avaliações. Jogos e tutor não retornam à superfície. | Mantém a direção visual pedida sem reintroduzir funcionalidades retiradas; a referência orienta hierarquia e ritmo, não cópia literal de conteúdo ou dados. |
