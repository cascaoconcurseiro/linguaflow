...
|---|---|---|---|---|
| P0 | Versionar migrations já aplicadas e eliminar divergência Git↔Supabase | Codex | Em andamento | As cinco migrations históricas e as de Codex já foram recuperadas; falta replay em banco efêmero e recuperar a migration Web Push aplicada por Fable fora do Git. |
...
| P2 | Web Push do PWA com consentimento | Codex | Em correção | Schema/Vault/cron foram iniciados por Fable na produção; cron foi pausado pois a função de envio não existe. Falta cliente, Service Worker, Edge Function e teste real. |
