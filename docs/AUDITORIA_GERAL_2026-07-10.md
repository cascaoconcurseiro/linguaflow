...
- **2026-07-12 — revisão independente da entrega Claude / produção `main` (`f040de6`): NÃO aprovada como “projeto terminado”.** A inspeção de UX, didática e vídeo confirmou as dores reportadas em uso real. No mobile, o `#app-root` já rola e a tela de estudo acrescenta áreas com `height:100%` e `overflow`, criando rolagem concorrente: é a causa estrutural do card parecer metade fixo/metade móvel. O card também disputa a tela com mídia, YouGlish, Tatoeba, chunks e tutor; no mobile todos são empilhados e o tutor fica fora do fluxo principal. O player atual usa a IFrame API, mas persiste apenas o início do trecho, descarta `cue.end`, prepara o vídeo sem ciclo confiável de carregamento/reprodução e não tem `endSeconds`, pausa automática, replay nem proteção contra corrida entre dois cards. Portanto ele não pode parar na frase. O motor FSRS é uma base aproveitável, mas não é paridade Anki: a cota de revisões também conta respostas novas/learning, cards de learning vencidos vão para o fim da fila, o `learn ahead` de 20 min encurta os steps, e os intervalos mostrados podem divergir da programação salva por causa do fuzz aleatório. Há ainda controles de configuração que não exercem efeito. **Nenhum desses itens foi marcado como concluído.** A correção será conduzida em blocos: (1) uma única rolagem e shell de revisão mobile, (2) player de trecho com início/fim persistidos e replay, (3) tutor/mídia como ações pós-revelação, (4) fila/limites/previsões do motor, (5) validação em aparelho real e regressão. Referência técnica para o player: IFrame API do YouTube (`loadVideoById` com `startSeconds`/`endSeconds`, estado `CUED`, `origin` e dimensão mínima 200×200).
