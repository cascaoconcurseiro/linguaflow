...
const ENDPOINT = "deepseek-v4-flash";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!token) {
      return new Response(JSON.stringify({ error: "Não autenticado. Faça login no LinguaFlow." }), { status: 401, headers: cors });
    }
    ...
    if (userErr || !userData?.user) {
      return new Response(JSON.stringify({ error: "Sessão inválida ou expirada. Faça login novamente." }), { status: 401, headers: cors });
    }
    ...
    if (body.action === "assess_pronunciation") {
        ...
        for (const model of audioModels) {
          ...
          try {
            audioResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", {
              ...
              method: "POST",
              headers: { "Authorization": `Bearer ${openRouterKey}`, "Content-Type": "application/json", "HTTP-Referer": "https://linguaflow-web-tau.vercel.app", "X-Title": "LinguaFlow" },
              body: JSON.stringify({ model, messages: [{ role: "user", content: [
                { type: "text", text: prompt }, { type: "input_audio", input_audio: { data: audio, format } },
              ] }], temperature: 0.1, max_tokens: 350, stream: false }),
            });
            providerData = await audioResponse.json().catch(() => null);
          } catch (error) {
            providerErrors.push({ model, attempt, status: 0, message: (error as Error)?.name || "network_error" });
            continue;
          }
          if (audioResponse.ok) break;
          providerErrors.push({ model, attempt, status: audioResponse.status, message: String(providerData?.error?.message || "resposta sem detalhe").slice(0, 300) });
        }
        if (audioResponse?.ok) break;
      }
      if (!audioResponse?.ok) {
        console.error("[voice-assessment] OpenRouter failures", providerErrors);
        return new Response(JSON.stringify({ available: false, fallback: "playback", feedback: "A avaliação automática está temporariamente indisponível. Ouça sua gravação e compare com o modelo." }), { status: 200, headers: cors });
      }
      ...
      try {
        ...
        return new Response(JSON.stringify({ score: Math.round(score), transcript: String(assessment.transcript || "").slice(0, 500), feedback: String(assessment.feedback || "").slice(0, 500), missed_words: Array.isArray(assessment.missed_words) ? assessment.missed_words.map(String).slice(0, 12) : [] }), { status: 200, headers: cors });
      } catch {
    ...
    if (!validTextMessages(body.messages)) {
      return new Response(JSON.stringify({ error: "Pedido de IA inválido.", code: "messages_invalid" }), { status: 400, headers: cors });
    }
    const quotaResponse = await consumeQuota(admin, userId, cors);
    if (quotaResponse) return quotaResponse;
    ...
    if (!DEEPSEEK_API_KEY && !OPENROUTER_API_KEY) {
      return new Response(JSON.stringify({ error: "Nenhum provedor de IA está configurado no servidor.", code: "no_provider" }), { status: 500, headers: cors });
    }
    ...
    const callProvider = (url: string, key: string, model: string) => {
      ...
      return fetch(url, {
        ...
        headers: {
          ...
          "Content-Type": "application/json",
          ...(url.includes("openrouter") ? { "HTTP-Referer": "https://linguaflow-web-tau.vercel.app", "X-Title": "LinguaFlow" } : {}),
        },
        body: JSON.stringify({ ...payload, model, ...(url.includes("openrouter") ? { reasoning: { effort: "minimal", exclude: true } } : {}) }),
      });
    };
    ...
    if (preferEconomy && OPENROUTER_API_KEY) {
      ...
      if ((!response || !response.ok) && DEEPSEEK_API_KEY) {
        try { response = await callProvider("https://api.deepseek.com/chat/completions", DEEPSEEK_API_KEY, "deepseek-v4-flash"); }
        catch (e) { primaryError = (e as Error).message; }
      }
    } else if (DEEPSEEK_API_KEY) {
      try {
        response = await callProvider("https://api.deepseek.com/chat/completions", DEEPSEEK_API_KEY, "deepseek-v4-flash");
      } catch (e) {
        primaryError = (e as Error).message;
      }
    }
    ...
    if (!response || !response.ok) {
      if (response && !response.ok) {
        ...
        primaryError = String(providerBody?.error?.message || primaryError || `HTTP ${response.status}`).slice(0, 300);
      }
      if (OPENROUTER_API_KEY) {
        try {
          ...
          if (!primaryError) primaryError = (e as Error).message;
        }
      }
    }

    if (!response) {
      return new Response(JSON.stringify({ error: `IA indisponível no momento (${primaryError || "falha de rede"}). Tente de novo em instantes.`, code: "provider_network_error" }), { status: 502, headers: cors });
    }

    if (!response.ok) {
      const providerBody = await response.clone().json().catch(() => null);
      const status = response.status;
      const detail = String(providerBody?.error?.message || primaryError || `HTTP ${status}`).slice(0, 300);
      console.error("[deepseek-chat] provider_unavailable", { status, detail, primaryStatus });
      const friendly = status === 401
        ? "A chave da IA é inválida ou não foi aceita pelo provedor."
        : status === 402
        ? "A conta da IA está sem saldo."
        : status === 429
        ? "O limite de requisições da IA foi atingido. Tente novamente em instantes."
        : status === 503
        ? "O provedor de IA está temporariamente sobrecarregado."
        : "O provedor de IA está temporariamente indisponível.";
      return new Response(JSON.stringify({ error: friendly, code: "provider_error", provider_status: status, detail }), { status: 502, headers: cors });
    }
