-- Codex 2026-07-12: o limite diário de revisões só deve contar respostas
-- dadas a cards que JÁ eram review/mature. New e learning são passos do mesmo
-- processo de aquisição e não podem esgotar o orçamento de revisão.
--
-- O status anterior é capturado dentro da RPC com lock da linha, nunca aceito
-- do navegador. Eventos históricos ficam NULL por não haver como reconstruir
-- seu estado anterior de forma confiável; a contagem correta vale para eventos
-- gravados após esta migration.

alter table public.review_log
  add column if not exists previous_status text;

create index if not exists review_log_user_previous_status_ts_idx
  on public.review_log (user_id, previous_status, ts);
...
declare
  ...
  v_previous_card public.cards%rowtype;
...
begin
  ...
  -- Captura o estado antes do update sob lock. Além de tornar a contagem de
  -- limite diário fiel, isto fecha a janela entre ler e atualizar o card.
  select * into v_previous_card
  from public.cards
  where id = p_card_id and user_id = v_user_id
  for update;
  if not found then raise exception 'card not found'; end if;

  -- Os campos aceitos são explicitamente listados para não transformar o JSON
  -- em PATCH livre.
  update public.cards
  ...
  insert into public.review_log
    (user_id, card_id, quality, date, ts, client_review_id, previous_status)
  values
    (v_user_id, p_card_id, p_quality, current_date, now(), p_client_review_id,
      v_previous_card.status)
