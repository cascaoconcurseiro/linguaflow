-- A transição SRS é calculada sob o mesmo lock que admite e grava a revisão.
-- p_state continua na assinatura para clientes publicados, mas é ignorado.

ALTER FUNCTION public.record_card_review(uuid, smallint, jsonb, uuid)
  RENAME TO record_card_review_before_server_fsrs;

REVOKE ALL ON FUNCTION public.record_card_review_before_server_fsrs(uuid, smallint, jsonb, uuid)
...
BEGIN
  ...
  BEGIN v_new_limit := greatest(0, least(20, coalesce(nullif(btrim(v_setting), '')::integer, 20)));
  ...
  BEGIN v_review_limit := greatest(1, least(1000, coalesce(nullif(btrim(v_setting), '')::integer, 200)));
  ...
  BEGIN v_leech_threshold := greatest(1, least(100, coalesce(nullif(btrim(v_setting), '')::integer, 8)));
  ...
  BEGIN v_easy_int := greatest(0.0001, coalesce(nullif(btrim(v_setting), '')::double precision, 4));
  ...
  BEGIN v_max_int := greatest(1, least(365000, coalesce(nullif(btrim(v_setting), '')::double precision, 36500)));
  ...
  BEGIN v_int_mod := greatest(0.01, least(10, coalesce(nullif(btrim(v_setting), '')::double precision / 100, 1)));
  EXCEPTION WHEN invalid_text_representation OR numeric_value_out_of_range THEN v_int_mod := 1; END;

  -- Somente estas três configurações admitem override por categoria.
  ...
  BEGIN v_grad_int := greatest(0.0001, coalesce(nullif(btrim(v_setting), '')::double precision, 1));
  ...
  BEGIN v_retention := greatest(0.7, least(0.97, coalesce(nullif(btrim(v_setting), '')::double precision, 0.9)));
  ...
  ELSIF v_card.status = 'new' THEN
    SELECT count(*)::integer INTO v_new_today FROM public.learning_events
     WHERE user_id=v_user_id AND event_type='card_reviewed' AND eligible
       AND eligibility_reason='due_new' AND local_date=v_today;
  ...
  ELSIF v_card.status IN ('review','mature') THEN
    SELECT count(*)::integer INTO v_review_today FROM public.review_log
     WHERE user_id=v_user_id AND date=v_today AND previous_status IN ('review','mature');
  ...
  IF v_eligible THEN
    ...
    IF v_card.status IN ('new','learning') THEN
      ...
      IF v_stability IS NULL THEN v_stability:=greatest(0.1,v_fsrs_w[p_quality]); END IF;
      ...
      ELSIF p_quality=4 THEN
        ...
        v_interval:=greatest(v_easy_int,(v_stability/v_factor)*(power(v_retention,1/v_decay)-1)*v_int_mod);
      ELSE
        v_step:=CASE WHEN v_card.status='new' THEN 1 ELSE v_step+1 END;
        IF v_step>=cardinality(v_active_steps) THEN v_status:='review'; v_step:=0; v_interval:=greatest(v_grad_int,(v_stability/v_factor)*(power(v_retention,1/v_decay)-1)*v_int_mod);
        ELSE v_status:='learning'; v_interval:=v_active_steps[v_step+1]/1440; END IF;
      END IF;
    ELSE
      v_stability:=coalesce(v_stability,greatest(coalesce(v_card.interval,1),0.1));
      v_difficulty:=coalesce(v_difficulty,greatest(1,least(10,v_fsrs_w[5])));
    ...
    UPDATE public.cards SET status=v_status, interval=v_interval, ease_factor=coalesce(v_card.ease_factor,2.5),
      ...
      suspended=v_card.suspended OR (v_leech_action='suspend' AND NOT v_card.is_leech AND v_lapses>=v_leech_threshold)
