...
ALTER FUNCTION public.record_card_review(uuid, smallint, jsonb, uuid)
  RENAME TO record_card_review_before_fsrs_hardening;

REVOKE ALL ON FUNCTION public.record_card_review_before_fsrs_hardening(uuid, smallint, jsonb, uuid)
...
BEGIN
  ...
  BEGIN
    IF lower(btrim(coalesce(v_setting,''))) IN ('nan','inf','+inf','-inf','infinity','+infinity','-infinity') THEN
      v_setting := NULL;
    END IF;
