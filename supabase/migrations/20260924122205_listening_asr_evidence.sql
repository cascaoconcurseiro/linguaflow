-- Original YouTube ASR is a qualified estimate of spoken language; manual or translated captions never qualify.
ALTER TABLE public.listening_intervals DROP CONSTRAINT listening_intervals_evidence_check;
ALTER TABLE public.listening_intervals ADD CONSTRAINT listening_intervals_evidence_check CHECK (evidence IN ('user_confirmed', 'audio_track', 'caption_asr'));
...
begin
  ...
  if p_event_id is null or p_seconds is null or p_seconds not between 1 and 60
    ...
    or p_evidence is null or p_evidence not in ('user_confirmed', 'audio_track', 'caption_asr') then
